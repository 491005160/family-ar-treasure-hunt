...
面向手机竖屏的单页寻宝原型。创建者可设置 1～4 个宝藏并生成家庭分享链接；正式玩法只截取中央识别框，使用浏览器本地 MobileNet 图像 embedding 与已启用目标比较。调试模式仍保留可预测的 mock matcher，用于验证进度和完成页。

开始页的四个槽各可选择一张自定义宝藏照片，未设置的槽显示 × 且不参与本局。照片会压缩后写入分享链接的 URL 片段，不上传服务器；接收者打开链接后进入只读游玩模式。
